...
function runDijkstra() {
    if (!startPoint || !endPoint) {
        ...
        logStep('❌ Need start and end points. Click "Load Sample City" and then click twice on the map.');
        return;
    }
    
    // Clear previous results
    document.getElementById('steps').innerHTML = '';
    logStep(`🔍 Requesting multiple route options (${travelMode.toUpperCase()} mode)...`);
    ...
    if (window.L && window.L.mapquest) {
        ...
        routeTypes.forEach((routeConfig) => {
            ...
            dir.route({
                ...
                if (error && Object.keys(error).length > 0 && error.message) {
                    logStep(`❌ Error getting ${routeConfig.label}: ` + error.message);
                } else if (!response || !response.route) {
                    logStep(`❌ No ${routeConfig.label} data received`);
                } else {
                    // Draw this route
        ...
        logStep('❌ MapQuest Directions not available');
    }
}
...
function buildCombinedGraphFromRoutes(routeResults) {
    ...
    logStep('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    logStep(`📊 Analyzing ${routeResults.length} different routes from MapQuest`);
    logStep('');
...
function runDijkstraVisualization(route) {
    ...
    if (graphResult) {
        logStep('✓ Using MapQuest maneuver data (exact distances & times)');
        logStep(`📊 MapQuest Route: ${graphResult.totalDistance.toFixed(2)} km, ${Math.round(graphResult.totalTime / 60)} min`);
        logStep('');
        useManeuvers = true;
    } else {
...
async function visualizeDijkstra(graph, nodeIds, startNodeId, endNodeId, useManeuvers = false, routeGraphs = null) {
    ...
    if (useManeuvers) {
        ...
        logStep('');
        logStep(`✓ Road distance matches MapQuest's reported route!`);
    }
    ...
    logStep('');
    logStep('💡 Summary: Dijkstra found the shortest path using MapQuest road');
